...
public class CarDetails {
    ...
    @PostConstruct
    public void create() {
        String trace = "CarDetails::create::@PostConstruct";
        try {
    ...
    public String countMessage(int count) {
        String trace = "CarDetails::countMessage::";
        try {
            traceG.traceDoc(trace);
        } catch (IOException ex) {
        ...
        if (count == 0) {
            return "No hay coches en la flota";
        } else {
            return "Disponemos de " + count + " coches de esta marca-modelo en la flota";
        }
    }

    @PreDestroy
    public void preDestroy() {
        String trace = "CarDetails::preDestroy::@PreDestroy";
        try {
            logApp = (LogApp) InitialContext.doLookup("java:global/CarHireEE/CarHireEE-ejb/LogApp!singletonBeans.LogApp");
            traceG.traceDoc(trace);
            logApp.logGenerator(trace);
